...
export async function GET(
  ...
  try {
    ...
    const apiUrl = `${baseUrl}/api/Locations/areas/${countryId}`;

    console.log('Fetching areas for country from:', apiUrl);
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: `Failed to fetch areas: ${response.status} ${response.statusText}` },
