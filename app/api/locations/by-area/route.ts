...
export async function GET() {
  try {
    ...
    const apiUrl = `${baseUrl}/api/Locations/by-area`;

    console.log('Fetching locations by area from:', apiUrl);
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: `Failed to fetch locations by area: ${response.status} ${response.statusText}` },
