...
export async function GET(request: NextRequest) {
  try {
    ...
    const apiUrl = `${baseUrl}/api/AccommodationList${queryString ? `?${queryString}` : ''}`;

    console.log('Fetching accommodations from:', apiUrl);
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: `Failed to fetch accommodation list: ${response.status} ${response.statusText}` },
        { status: response.status }
      );
    }

    const data = await response.json();
    console.log('Successfully fetched accommodations, count:', Array.isArray(data) ? data.length : 'not an array');
